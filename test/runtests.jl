...
@fixture full_dfs function(file, filename)
  ret = []
  for f in [file, filename]
    push!(ret, readxl(DataFrame, f, "Sheet1!C3:O7"))
    push!(ret, readxlsheet(DataFrame, f, "Sheet1"))
  end
  ret
end
...
@testset "ExcelReaders tests" begin
  ...
  @pytest function reading_toarray(file, filename)
  ...
  @pytest function reading_noheader(file, filename, good_colnames)
    ...
    for f in [file, filename]
        ...
        df = readxl(DataFrame, f, "Sheet1!C4:O7", header=false, colnames=good_colnames)
        @test ncol(df) == 13
        @test nrow(df) == 4
  ...
  @pytest function overriding_colnames(file, filename, good_colnames)
    for f in [file, filename]
        full_dfs = [readxl(DataFrame, f, "Sheet1!C3:O7", header=true, colnames=good_colnames),
                    readxlsheet(DataFrame, f, "Sheet1", header=true, colnames=good_colnames)]
        for df in full_dfs
            @test ncol(df) == 13
            @test nrow(df) == 4
            @test isa(df[:c1], DataVector{Float64})
  ...
  @pytest function toofewcolnames(file, filename)
    for f in [file, filename]
  ...
  @pytest function read_empty_xlsheet(file, filename)
    for f in [file, filename]
      @test_throws ErrorException readxlsheet(f, "Empty Sheet")
    end
  end

  @pytest function readxlsheet_bad_dims(file, filename)
    for f in [file, filename]
  ...
  @pytest function readxlsheet_succesful(file, filename)
    for f in [file, filename]
      for sheetinfo=["Second Sheet", 2]
